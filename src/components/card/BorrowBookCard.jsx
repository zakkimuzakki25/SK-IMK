...
const BorrowBookCard = ({pic, title, rate, tot_review}) => {
    ...
    return (
        <div className="flex flex-col p-5 gap-2 border border-zinc-300 rounded-md shadow-lg bg-white">
            <Link className="flex py-5 w-52 h-64 overflow-hidden justify-center bg-backgroundsolid hover:bg-primary2 hover:bg-opacity-80 hover:py-4 transition-all">
            ...
            <div className="flex-row flex gap-2">
                <div className="w-full flex"><OtherButtonRounded name={"renew"}/></div>
                <div className="w-full flex"><PrimerButtonRounded name={"return"}/></div>
            </div>
        </div>
    )
}
